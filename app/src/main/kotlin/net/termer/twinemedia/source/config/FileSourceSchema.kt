...
/**
 * Configuration schema for file source configs
 * @author termer
 * @since 1.5.0
 */
class FileSourceSchema(
	...
	override fun toJson() = jsonObjectOf(
		"sections" to JsonObject().apply {
			for(section in sections)
				put(section.sectionName, jsonObjectOf(
					"name" to section.name
				))
		},
		"fields" to JsonObject().apply {
			for(field in fields)
				put(field.fieldName, jsonObjectOf(
					"name" to field.name,
	...
	private fun isType(value: Any?, type: Field.Type) = when {
		value == null -> false
		type == Field.Type.DATE -> value is String && value.matches(datePattern)
		type == Field.Type.NUMBER -> value is Byte || value is Short || value is Int || value is Long || value is Float || value is Double
		else -> {
	...
	/**
	 * Data class that holds information about schema validation responses
	 * @author termer
	 * @since 1.5.0
	 */
	...
	/**
	 * Data class that contains info about a schema section
	 * @author termer
	 * @since 1.5.0
	 */
	data class Section(
	...
	/**
	 * Data class that contains info about a schema field
	 * @author termer
	 * @since 1.5.0
	 */
	data class Field(
