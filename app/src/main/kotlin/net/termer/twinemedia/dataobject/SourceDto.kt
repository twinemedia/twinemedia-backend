...
class SourceDto(
    ...
    /**
	 * The file source's owner, or null if the account no longer exists
	 * @since 2.0.0
	 */
	...
	companion object {
		...
		fun fromRow(row: Row): SourceDto {
			val sourceOwnerId = row.getString("source_owner_id")
			...
			return SourceDto(
				...
				owner = if(sourceOwnerId == null) null else RecordOwnerDto(
					id = sourceOwnerId,
					name = row.getString("source_owner_name")
				),
