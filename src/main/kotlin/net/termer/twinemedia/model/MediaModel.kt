...
class MediaModel {
     ...
     * @param params The array of SQL parameters to append values to
     * @return a default SQL WHERE filter to insert into a query
     * @since 1.2.0
     */
    private fun excludeTagsFilter(params: JsonArray): String {
        val sql = StringBuilder("TRUE")

        if(account != null) {
            for (tag in account!!.excludeTags) {
                sql.append(" AND NOT media_tags::jsonb ?? ?")
                params.add(tag)
            }
        }
    ...
    suspend fun fetchMedia(mediaId: String): ResultSet? {
        return client?.queryWithParamsAwait(
                "SELECT * FROM media WHERE ${viewWhereFilter()} AND media_id = ?",
    ...
    suspend fun deleteMedia(mediaId: String) {
        client?.queryWithParamsAwait(
                """
                DELETE FROM media WHERE ${deleteWhereFilter()} AND media_id = ?
            """.trimIndent(),
                JsonArray().add(mediaId)
        )
    }
