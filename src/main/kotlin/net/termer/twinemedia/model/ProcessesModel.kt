...
import net.termer.twinemedia.db.dataobject.Process
import net.termer.twinemedia.db.dataobject.ProcessInfo
...
class ProcessesModel {
	 ...
	 * @since 1.4.0
	 */
	suspend fun fetchProcess(id: Int): RowSet<Process> {
		return SqlTemplate
				.forQuery(client, """
					SELECT * FROM processes WHERE ${viewWhereFilter()} AND id = #{id}
				""".trimIndent())
				.mapTo(Process.MAPPER)
				.execute(hashMapOf<String, Any>(
						"id" to id
				)).await()
	}
	 ...
	 * @since 1.4.0
	 */
	suspend fun fetchProcessInfo(id: Int): RowSet<ProcessInfo> {
		return SqlTemplate
				.forQuery(client, """
					${infoSelect()}
					WHERE
	 ...
	 * @since 1.4.0
	 */
	suspend fun fetchProcesses(offset: Int, limit: Int, order: Int): RowSet<ProcessInfo> {
		return SqlTemplate
				.forQuery(client, """
					${infoSelect()}
	 ...
	 * @since 1.4.0
	 */
	suspend fun fetchProcessesForMime(mime: String): RowSet<ProcessInfo> {
		return SqlTemplate
				.forQuery(client, """
					${infoSelect()}
					WHERE
	 ...
	 * @since 1.4.0
	 */
	suspend fun fetchProcessesForMimeAndAccount(mime: String, account: Int): RowSet<ProcessInfo> {
		return SqlTemplate
				.forQuery(client, """
					${infoSelect()}
					WHERE
