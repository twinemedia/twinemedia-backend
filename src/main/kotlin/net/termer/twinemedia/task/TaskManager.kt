...
import net.termer.twinemedia.Module.Companion.sockJSManager
import net.termer.twinemedia.db.dataobject.Account
import net.termer.twinemedia.sockjs.SockJSClient
...
class TaskManager {
	...
	fun broadcastTaskStateChange(task: Task) {
		...
		for(client in clients)
			sockJSManager.broadcastEventByAccountId(client.account.id, "task_update", json {obj(
					"id" to task.id,
	...
	fun broadcastTaskDeleted(task: Task) {
		...
		for(client in clients)
			sockJSManager.broadcastEventByAccountId(client.account.id, "task_delete", json {obj(
					"id" to task.id
			)})
	}
	...
	fun broadcastTaskDeletedTo(task: Task, accountId: Int) {
		sockJSManager.broadcastEventByAccountId(accountId, "task_delete", json {obj(
				"id" to task.id
		)})
	}
